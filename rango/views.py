...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # Now hash the password with set_password method.
...
def user_login(request):
    # If the request is a HTTP POST, try to pull out to the relevant information.
    if request.method == 'POST':
        # Gather username and password provided by user.
        # This information is obtained from login form.
        # Use request.POST.get('<variable>') as opposed
        ...
        # If None (representing the absence of a value),
        # no user with matching credentials was found.
        if user:
            # Is the account active? It could have been disabled.
            if user.is_active:
                # If the account is valid and active, user can log in.
...
# Use login_required() decorator to ensure only those logged in
# can access the view.
@login_required
